...
var DemoList = class extends import_coc.BasicList {
  constructor(nvim) {
    super(nvim);
    this.name = "demo_list";
    this.description = "CocList for coc-list-vim-select";
...
async function activate(context) {
  import_coc2.window.showMessage(`coc-list-vim-select works!`);
  context.subscriptions.push(
    import_coc2.commands.registerCommand("coc-list-vim-select.Command", async () => {
      import_coc2.window.showMessage(`coc-list-vim-select Commands works!`);
    }),
    import_coc2.listManager.registerList(new DemoList(import_coc2.workspace.nvim)),
    import_coc2.sources.createSource({
      name: "coc-list-vim-select completion source",
      doComplete: async () => {
